...
public class GetFunctionsByUserQueryHandler : IQueryHandler<GetFunctionsByUserQuery, List<GetFunctionsByUserResponse>>
{
    ...
    public GetFunctionsByUserQueryHandler(
    ...
    public async Task<Result<List<GetFunctionsByUserResponse>>> Handle(GetFunctionsByUserQuery request, CancellationToken cancellationToken)
    {
        ...
        var functions = await GetFunctionsByUserAsync(user);
        var result = _mapper.Map<List<GetFunctionsByUserResponse>>(functions);
        return Result.Success(result);
    }

    private async Task<List<Domain.Entities.Function>> GetFunctionsByUserAsync(Domain.Entities.User user)
    {
        // 1. Lấy map PermissionKey -> int ActionValue user có
        var permissionIntMap = await GetPermissionIntByUserAsync(user);

        // 2. Lấy tất cả function đang Active (có thể thêm điều kiện Status == Active)
        var allFunctions = await _dbContext.Functions
            .Where(f => f.Status == Status.Active)
            .OrderByDescending(f => f.SortOrder)
            .ToListAsync();

        // 3. Lọc các function user có quyền
        var available = allFunctions
            .Where(f =>
                !string.IsNullOrEmpty(f.Key) &&
                permissionIntMap.TryGetValue(f.Key, out var userActionValue) &&
                HasPermissionForFunction(userActionValue, f.ActionValue))
            .ToList();
    ...
    /// functionActionValue: bitmask yêu cầu để thấy menu (VD: 1 = View, hoặc 3 = View+Add)
    /// </summary>
    private bool HasPermissionForFunction(int userActionValue, int functionActionValue)
    {
        // Phase 1: chỉ cần user có ÍT NHẤT 1 quyền trùng với yêu cầu:
        return (userActionValue & functionActionValue) != 0;

        // Phase 2: bắt buộc user phải có ĐỦ toàn bộ bit mà function yêu cầu:
        //return (userActionValue & functionActionValue) == functionActionValue;
    }
